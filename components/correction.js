        ...
        Exam.getExam(subExam, function (err, exam) {
            ...
            questionsId.forEach(function (id) {
                ...
                Question.getQuestion(id, function (err, question) {
                    console.log('Getting question (id: '+question._id+'): '+JSON.stringify(question)); // TEST
...
module.exports.autoCorrect = function(question, submittedExam, orgExam, callback) {
    console.log(JSON.stringify(question)); // TEST
    ...
    if (submittedExam.completeCorrection != true) {
        for (var i = 0; i < question.length; i++) {
            //console.log('Current Question (index '+i+')'+JSON.stringify(question[i])); // TEST
            ...
            if(type === 'single') {
                ...
                //console.log('Current answer is single (index '+i+') : '+JSON.stringify(subAnswers)); // TEST
                if(!subAnswer[0].corrected) {
                    for (var j = 0; j < question[i].answerOptions.length; j++) {
                        if (subAnswer[0].text === question[i].answerOptions[j].text && question[i].answerOptions[j].correct && !foundAnswer) {
            ...
            // Multi type
                ...
                //console.log('Current answer is multi (index '+i+') : '+JSON.stringify(subAnswers)); // TEST
                var correctArray = [];
                ...
                console.log(JSON.stringify(correctArray)); // TEST
                for (var j=0; j<subAnswers.length; j++) {
                    if (!subAnswers[j].corrected) {
                        // console.log('Subanswer '+JSON.stringify(subAnswers[j])+' deemed not corrected Index i '+i+' index j '+j); // TEST
                        if (correctArray.indexOf(subAnswers[j].text) > -1) {
                            // console.log('Subanswer '+JSON.stringify(subAnswers[j])+' is correct'); // TEST
                            ...
                            // console.log('Subanswer '+JSON.stringify(subAnswers[j])+' is NOT correct'); // TEST
                            subAnswers[j].corrected = true;
            ...
            // Rank type
                ...
                var subAnswers = submittedExam.answers[i];
                // console.log('Current answer is rank (index '+i+') : '+JSON.stringify(subAnswers)); // TEST
                for (var j = 0; j < subAnswers.length; j++) {
                    if(!subAnswers[j].corrected) {
                ...
                // console.log('Current answer is text (index '+i+') : '+JSON.stringify(subAnswers)); // TEST
            }
        }
